...
def _build_init_from_bruteforce(
    ...
    # Apply axis-swap correction if brute-force used a different detector-axis
    # convention.  Here omega is Ω, the longitude of the ascending node.
    # The transformation maps (i, Ω) → (π − i, −Ω + π/2).
...
def compute_projection_matrices_from_hkpq(
    ...
    # the standard sky-projection matrix for (Ω, i, ω) and must not be swapped.
    w     = np.arctan2(h, k)   # Ω + ω
