...
#   h   = e · sin(ω + θ0)            — eccentricity vector component
#   k   = e · cos(ω + θ0)            — eccentricity vector component
#   p   = sin(i/2) · cos(ω - θ0)     — inclination/orientation component
#   q   = sin(i/2) · sin(ω - θ0)     — inclination/orientation component
#
# From (h, k) you can recover:  e = √(h²+k²),  ω+θ0 = atan2(h, k)
# From (p, q) you can recover:  i = 2·arcsin(√(p²+q²)),  ω-θ0 = atan2(q, p)
#
...
# (x_k, y_k) in native pixels, then extract a photometric scalar from the
...
#
# With a single instrument the result is exactly the original single-instrument
# KStacker behavior.
#
# ─────────────────────────────────────────────────────────────────────────────
...
def _build_init_from_bruteforce(
    ...
    (a, e, t0, m0, ω, i, θ0), which this function converts to the
    non-singular sampling space (a, λ0, m0, h, k, p, q).
    ...
    If `bruteforce_swap_xy: true` in the YAML, a coordinate-axis swap
    transformation is applied to (i, ω) before the conversion.
    """
    ...
    if data.ndim != 2 or data.shape[1] < 7:
        raise ValueError(
            ...
            "expected (N, ≥7) with columns (a, e, t0, m0, ω, i, θ0)."
        )
    ...
    # Apply axis-swap correction if brute-force used a different orientation
    # convention.  The transformation maps (i, ω) → (π − i, −ω + π/2).
    if bool(root.get("bruteforce_swap_xy", False)):
        inc = np.pi - inc
        omega = wrap_2pi(-omega + 0.5 * np.pi)
...
def _resolve_parallel(root: dict) -> dict:
    ...
    max_workers : number of worker processes for chunked log-prob evaluation.
                  None (default) → let Python choose (usually = CPU count).
    chunk_size  : number of walkers evaluated per submitted job.
                  Larger = fewer submissions but larger memory footprint per job.
    """
    par = root.get("parallel", {}) or {}
    max_workers = par.get("max_workers", None)
    if isinstance(max_workers, str) and max_workers.strip().lower() == "none":
        max_workers = None
    return dict(
        max_workers=None if max_workers is None else int(max_workers),
        chunk_size =int(par.get("chunk_size", 256)),
    )
...
def compute_projection_matrices_from_hkpq(
    ...
    The matrix R maps (x_orb, y_orb) in the orbital plane [AU] to (x_sky, y_sky)
    in the sky plane [AU].

    Derivation
    ──────────
        ω      = (atan2(h,k) + atan2(q,p)) / 2    (argument of periapsis)
        θ0     = (atan2(h,k) − atan2(q,p)) / 2    (argument of ascending node)
...
def snr_from_hkpq(
    ...
    *,
    ...
    w_ok  = np.arctan2(h_ok, k_ok)                            # ω + θ0
    ...
    x_sky = x_orb * r00 + y_orb * r01
    y_sky = x_orb * r10 + y_orb * r11

    cx = cy = size // 2
    x_pix = x_sky * scale + cx       # (W_ok, K) native pixel x
    y_pix = y_sky * scale + cy       # (W_ok, K) native pixel y
...
def flux_sufficient_stats_from_hkpq(
    ...
    *,
    ...
    cx = cy = size // 2
    x_pix = x_sky * scale + cx
    y_pix = y_sky * scale + cy
...
def _logprob_chunk(thetas_chunk: np.ndarray, lp_kwargs: dict) -> np.ndarray:
    """
    Evaluate the vectorized log-posterior on one chunk of walkers.

    This thin wrapper exists so that it can be pickled and sent to a worker
    process by ProcessPoolExecutor.
    """
...
class BatchPoolLogProb:
    ...
    emcee calls this object with all W walkers at once.  We split the batch
    into chunks of size `chunk_size` and submit them to a ProcessPoolExecutor
    for parallel evaluation.

    When W ≤ chunk_size, the evaluation happens in-process (no pickling overhead).

    Parameters
    ──────────
    lp_kwargs  : keyword arguments for `log_probability`.
    executor   : ProcessPoolExecutor instance.
    chunk_size : number of walkers per submitted job.
    """
    ...
    def __call__(self, thetas: np.ndarray) -> np.ndarray:
        ...
        if W <= self.chunk_size:
            return _logprob_chunk(thetas, self.lp_kwargs)

        futures, slices = [], []
        for start in range(0, W, self.chunk_size):
            stop = min(start + self.chunk_size, W)
            fut  = self.executor.submit(_logprob_chunk, thetas[start:stop], self.lp_kwargs)
            futures.append(fut)
...
def run_emcee_hybrid(
    ...
    *,
    ...
    """
    Run emcee with vectorized log-probability and chunked multiprocessing.

    Workflow
    ────────
    1. Open a ProcessPoolExecutor with `max_workers` processes.
    2. Wrap the log-prob function in BatchPoolLogProb.
    3. Run `burnin` steps; call sampler.reset() to discard burn-in.
    4. Run `nsteps` production steps.
    5. Return the sampler (production chain only).

    The default moves are a 50/50 mix of DEMove and DESnookerMove, which
    work well for multi-modal or curved posteriors.
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        batched  = BatchPoolLogProb(lp_kwargs, executor, chunk_size)
        sampler  = emcee.EnsembleSampler(
            p0.shape[0], p0.shape[1], batched, vectorize=True, moves=moves
        )
...
def derive_physical_from_chain(a, la0, m0, h, k, p, q, t_ref):
    ...
    Returns (M0, t0, e, inc, omega, theta0) for each sample.
    """
    w      = np.arctan2(h, k)        # ω + θ0
    Delta  = np.arctan2(q, p)        # ω − θ0
    omega  = 0.5 * (w + Delta)
...
def _predict_pixel_track_native(theta, ts, *, size, scale, t_ref):
    ...
    x_sky = x_orb * proj[0, 0] + y_orb * proj[0, 1]
    y_sky = x_orb * proj[1, 0] + y_orb * proj[1, 1]
    cx = cy = size // 2
    return x_sky * scale + cx, y_sky * scale + cy
...
def _predict_tracks_native_chunk(thetas_chunk, ts, *, size, scale, t_ref):
    ...
    Returns (x_pix, y_pix) of shape (K, Wc).
    """
    ...
    x_sky = x_orb * R[None, :, 0, 0] + y_orb * R[None, :, 0, 1]
    y_sky = x_orb * R[None, :, 1, 0] + y_orb * R[None, :, 1, 1]

    x_pix = x_sky * scale + (size // 2)
    y_pix = y_sky * scale + (size // 2)
    return x_pix, y_pix
...
def _offtrack_stat_for_theta(
    ...
    *,
    ...
    for inst in instruments:
        ...
        x_sky_ = x_orb_ * proj[0, 0] + y_orb_ * proj[0, 1]
        y_sky_ = x_orb_ * proj[1, 0] + y_orb_ * proj[1, 1]
        cx = cy = float(inst.size // 2)
        base_tracks[inst.name] = (x_sky_ * inst.scale + cx, y_sky_ * inst.scale + cy)
    ...
    for _ in range(int(n_trials)):
        ...
        if use_abs:
            ...
            u_global  = rng.uniform(1.0, 5.0, size=max_K)
            dr_global = None
...
def run_mcmc_from_yaml(yaml_path: str):
    ...
    try:
        run_glrt_offtracks_and_save(
            ...
            noise_floor=noise_floor,
            weighting=weighting,
            n_off=int(root.get("glrt", {}).get("n_off", 200)),
            seed=root.get("random_seed", None),
        )
